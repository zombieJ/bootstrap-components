...
trueenable close the modal on click backdrop.
focus:booleandefault is true. When show the target element as darklight, target element will focus
event:stringString: when element call the target function, dark background will hide.
...
start:booleandefault is null, false to prevent show right now.
...
timertime picker only
daterdate picker only
...
function formatDate(date, outer) {
var str_date = "";
...
/*this is to help hightlight target element with dark background.
options:
title:stringspecify title of dialog.
content:elementspecify content of dialog.
close:booleandefault is true for alert window contains close button
confirm:booleandefault is false for dialog can check yes or no
...
/* options:
type:oneenable only one file to upload[default]
mutienable muti files to upload
